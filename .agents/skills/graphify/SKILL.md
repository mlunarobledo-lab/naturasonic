---
name: graphify
description: "Grafo de conocimiento del codigo (Graphify): mapea arquitectura, dependencias, comunidades y blast-radius en `.graphify/`. Herramienta canonica de Praxis para mapear la realidad del codebase antes de planear o tocar codigo. Convierte codigo, docs, papers o imagenes en un grafo navegable con god nodes, deteccion de comunidades y consultas BFS/DFS. Activar ante cualquier pregunta de arquitectura o relaciones del proyecto, y para mantener el mapa fresco."
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, Agent, WebFetch
effort: high
---

# Skill: graphify — el grafo de conocimiento del proyecto

> Esta skill cumple el [STYLE-GUIDE de skills Praxis](../STYLE-GUIDE.md). El cuerpo de la receta (los comandos `graphify ...` y los `node -e` de la tuberia) es la operacion exacta de la herramienta `@sentropic/graphify`: se conserva verbatim porque es codigo ejecutable, no prosa traducible. La doctrina de abajo — como usa el agente el grafo dentro de Praxis — es lo que Praxis aporta.

## Por que Praxis nace graph-first

Todo proyecto Praxis trabaja desde un **grafo de conocimiento del codigo** que vive en `.graphify/`. El grafo es tu **mapa estructural de la realidad**: nodos (funciones, componentes, tablas, conceptos), aristas con auditoria honesta (EXTRACTED / INFERRED / AMBIGUOUS), comunidades detectadas y "god nodes" (los puntos mas conectados del sistema). Es la aplicacion directa del principio Praxis *"la precision viene de mapear la realidad, no de imaginar el futuro"*: el determinismo del grafo antes que la suposicion.

**Contrato de uso del agente:**

- **Orientarse antes de tocar codigo.** Ante cualquier pregunta de arquitectura, dependencias o "¿donde vive X y que lo toca?", si existe `.graphify/graph.json` tu **primer** movimiento es consultar el grafo — no un grep a ciegas:
  - `graphify summary --graph .graphify/graph.json` → orientacion de primer salto (barata) antes de traversar.
  - `graphify query "<pregunta>"` → subgrafo scopeado a la pregunta (BFS, contexto amplio); `--dfs` para trazar un camino concreto.
  - `graphify path "<A>" "<B>"` → camino mas corto entre dos conceptos; `graphify explain "<nodo>"` → explicacion en lenguaje llano.
  El grafo devuelve un subgrafo pequeño y preciso — casi siempre menos tokens que `GRAPH_REPORT.md` o que un grep crudo.
- **Medir el blast-radius antes de un cambio.** Antes de una fase o un PR, calcula que se vera afectado:
  - `graphify review-delta --files <archivos> --graph .graphify/graph.json` → impacto de los archivos que vas a cambiar.
...
- **Mantenerlo fresco.** El grafo es inutil si miente. Tras cambios de codigo, actualizalo — es barato (solo AST, sin LLM, cuando solo cambio codigo):
  - `graphify <path> --update` (re-extrae solo lo nuevo/cambiado), `graphify <path> --watch` (rebuild automatico en background entre olas de trabajo), o instala el hook post-commit con `graphify hook install`.
  - Si `.graphify/needs_update` existe o `.graphify/branch.json` marca `stale`, el grafo esta viejo: actualiza antes de confiar en resultados semanticos.
- **Construirlo cuando no existe.** En un proyecto recien scaffoldeado casi no hay codigo que graficar todavia: el grafo **crece con el proyecto**. Construyelo (o reconstruyelo) sobre el codebase real con la tuberia de abajo la primera vez que el mapeo de contexto lo justifique, y de ahi mantenlo fresco.

**El grafo es la fuente estructural; el grep la complementa.** Para scopes triviales (2-3 archivos) leerlos directo sigue siendo mas rapido; el grafo brilla cuando el mapeo cruza multiples archivos, comunidades o capas.

El resto de este documento es la operacion completa de Graphify (la herramienta canonica): tuberia de construccion, consultas, exports y mantenimiento.
...
This writes a `## graphify` section to the local `AGENTS.md` that instructs Claude to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.
