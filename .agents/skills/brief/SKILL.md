...
La doctrina completa vive en `@.agents/skills/bucle-agentico/SKILL.md`. Lee esa skill para entender el patron entero antes de ejecutar este brief.
...
El brief siempre alimenta **un solo PRP** que cubre todas sus fases (ver `@.agents/skills/prp/SKILL.md`). Se acabaron los conceptos de "brief master", "brief single", "PRP por fase" y "PRP unico monolitico": hay un brief, y de el sale un PRP con fases.
...
2. **Estado del workspace** usando Glob/Read:
   - ¿Existe `package.json`, `src/`, `.agents/`, `BUSINESS_LOGIC.md`, `BRIEF*.md`, `PRP*.md`?
...
5. Anunciar al usuario con dos componentes obligatorios — sintaxis correcta + plan de ejecucion explicito.
   ...
   **Componente A — Sintaxis correcta de Claude Code**:
   ...
   Para convertirlo en PRP: invoca @.agents/skills/prp/SKILL.md
...
La doctrina recursiva completa vive en `@.agents/skills/bucle-agentico/SKILL.md`.
