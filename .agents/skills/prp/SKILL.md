...
La doctrina completa vive en `@.agents/skills/bucle-agentico/SKILL.md`. Lee esa skill para entender el patron entero antes de generar este PRP.
...
3. Acumular **aprendizajes heredados** relevantes: del propio brief (si alguna fase ya tiene `Aprendizajes para fases siguientes` poblado de una corrida previa) + de `AGENTS.md` transversal. En el caso normal (brief recien creado) no hay aprendizajes previos.
...
2. `.agents/PRPs/prp-base.md` (fallback de compatibilidad para proyectos historicos).
...
1. Leer el workspace: `BUSINESS_LOGIC.md`, `README.md`, `AGENTS.md`, PRPs previos en `.agents/PRPs/`, `package.json`, ultimo `git log -10 --oneline`. Cualquier informacion inferible (objetivo del proyecto, restricciones tecnicas, contexto de negocio) sale de aqui.
...
**Nombre del archivo**: `.agents/PRPs/PRP-{numero}-{feature-kebab}.md`
...
```
Plan listo en .agents/PRPs/PRP-XXX-{kebab}.md.
...
Una vez aprobado, la implementacion se hace con `@.agents/skills/bucle-agentico/SKILL.md`, que:
...
- Al cerrar, propaga aprendizajes al PRP, al brief origen, y a AGENTS.md transversal.
...
- **SIEMPRE asignar numero** con `ls .agents/PRPs/` + `grep "PRP-" AGENTS.md` antes de crear el archivo.
