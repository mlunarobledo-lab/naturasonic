...
## Protocolo de Presencia Continua (Nexus Presence)

> El Director no existe solo cuando lo llaman. El Director SIEMPRE sabe en qué estado está el proyecto.

**Al inicio de CADA sesión, ANTES de atender la petición del usuario:**

1. **Despertar**: Lee `project-pulse.md` de la auto memory del proyecto. Si no existe, créalo con el template.
2. **Diagnóstico**: `git log --oneline -10` + `git status -s` + build check rápido (detecta stack automáticamente: gradle, npm, cargo, go, dotnet, make).
3. **Reporte**: Una frase al usuario con estado del proyecto, cambios desde la última sesión, y bloqueadores.
4. **Monitoreo**: Si la sesión será larga (>3 interacciones), armar cron de build cada 30 min vía CronCreate.

**Al CERRAR cada sesión (usuario dice "listo"/"gracias" o >10 interacciones):**

5. **Pulso**: Actualizar `project-pulse.md` con: qué se hizo, decisiones tomadas (con el POR QUÉ), bloqueadores, próximas acciones, estado del build.

**Excepción emergencia**: Si el usuario entra con un bug o algo roto, activar Protocolo EMERGENCIA (RAIDE: Reproducir, Aislar, Investigar, Despachar, Evidenciar). Despertar después.

**Deuda técnica**: Mantener `tech-debt.md` en auto memory. Registrar stubs, workarounds, y soluciones temporales al cerrar PRPs. Revisar items Alta en cada DESPERTAR.

**Reportes**: Ante "necesito un update / status / resumen", generar reporte en lenguaje de negocio (one-liner, párrafo, o completo). Protocolo REPORTE en la skill.

Skill canónica: `@.claude/skills/nexus-presence/SKILL.md`. Pre-commit hook instalado en `.git/hooks/pre-commit` (bloquea secrets, archivos prohibidos, binarios >5MB).
...
| "Estado del proyecto / qué pasó / monitorear / vigilar / presencia continua" | `nexus-presence` |
| "Métricas / velocidad / eficiencia / KPIs / rendimiento del proyecto" | `velocity` |
...
<!-- PRAXIS:SKILLS_START -->
## Skills: 12 Herramientas Especializadas
...
| 11 | `nexus-presence` | Presencia continua: wake-up, monitoreo en sesion, cierre con pulso cross-session |
| 12 | `velocity` | Metricas de velocidad: PRPs/semana, tiempo por PRP, retrabajo, tendencia |
<!-- PRAXIS:SKILLS_END -->
...
`-- skills/                       # 10 skills activos
    ...
    ├── prp/                   # Planes (PRPs)
    ├── nexus-presence/        # Presencia continua cross-session
...
**Cloud Sync & Encrypted Backup**: Arquitectura offline-first con dos workers. **ProfileSyncWorker** (@HiltWorker) consulta perfiles dirty vía DAO y sube vía CloudSyncApi. **BackupWorker** (@HiltWorker) serializa las 6 entities Room a JSON (kotlinx.serialization `@Serializable` en cada entity + `getAllForBackup()` en cada DAO), cifra con `BackupCryptoManager` (AES-256-GCM, IV random 12B prepended, passphrase de `KeyStoreManager`), y sube via `CloudSyncApi.uploadBackup(ByteArray)`. `CloudBackupManager` singleton Hilt coordina backup manual (`scheduleBackup()` → OneTimeWorkRequest) y automático (`schedulePeriodicBackup()` → PeriodicWorkRequest 24h con batería no baja + red). CloudSyncApi interface + StubCloudSyncApi (log-only, backend real pendiente — `uploadBackup` retorna `true` para UX). SyncManager encola ambos workers. NaturaSonicApp implementa Configuration.Provider con HiltWorkerFactory. `CloudBackupScreen` Compose con toggle auto-backup, fecha último respaldo, botón "Respaldar ahora" con feedback de estado (IDLE/IN_PROGRESS/SUCCESS/ERROR via WorkInfo observation). Acceso: Settings → "Copia de seguridad". Preferencias (autoBackupEnabled, lastBackupTimestamp) en DataStore. Para enchufar backend real: swap single binding en AppModule (provideCloudSyncApi).

**DSP Thread Watchdog**: Monitoreo de salud del hilo de audio nativo con recuperación automática. C++: `lastCallbackNs_` (std::atomic<int64_t>) actualizado en cada `onAudioReady` con `steady_clock::now()`, `accumulatedXRuns_` (std::atomic<int32_t>) acumula xRuns entre reinicios de stream, `restartCount_`/`consecutiveErrors_` (std::atomic<int32_t>) para telemetría de reinicios. `onErrorAfterClose` refactorizado con backoff exponencial (100ms base, 2x factor, cap 5s, max 5 reintentos) — `consecutiveErrors_` se resetea a 0 en cada `onAudioReady` exitoso. `WatchdogStats` struct + `getWatchdogStats()` + `resetWatchdog()` expuestos vía JNI (`nativeGetWatchdogStats` retorna jlongArray[5]). `DspWatchdogManager` singleton Hilt: polling cada 2s, stall detection (callback >500ms sin update + engine running), 2 checks consecutivos stale antes de forzar restart + emit `restartEvent: SharedFlow<Unit>`. AudioService observa `dspWatchdogEnabled` DataStore (default: true) para start/stop monitoring, y observa `restartEvent` para llamar `reapplyAllPreferences()` — lee valores actuales via `.first()` y los re-aplica al engine (profile, head tracking, AEC, transient limiter). `DspWatchdogScreen` Compose con indicador de salud color-coded (verde=GOOD, amarillo=WARNING, rojo=CRITICAL), stats de xRuns/reinicios/callback age/errores consecutivos, botón "Forzar reinicio" y "Resetear contadores". Acceso: Settings → "Motor de audio". Preferencia `dspWatchdogEnabled` en DataStore.
